"""
Qdrant Memory MCP Server V2 — 增强版
改进：importance 分级、向量+关键词混合搜索、结果去重、加权排序
向后兼容 v1 数据（无 importance 字段的记录视为 medium）
"""
...
mcp = FastMCP(
    "Claude Memory V2",
    instructions="""
这是 Claude 的永久向量记忆系统 V2（增强版）。改进：

1. **store_memory**: 存储记忆，自动根据分类设定重要性等级（high/medium/low）
2. **search_memory**: 智能搜索，支持向量语义 + 关键词混合，重要记忆优先展示
3. **keyword_search**: 精确关键词搜索，适合搜特定术语、项目名、工具名
4. **delete_memory**: 删除过时或错误的记忆
...
def search_memory(query: str, category: str = "", top_k: int = 5) -> str:
    ...
    for point in results.points:
        ...
        importance = payload.get("importance", "medium")  # 兼容 v1 数据
...
def keyword_search(keyword: str, category: str = "", limit: int = 5) -> str:
    ...
    if not results:
        ...
        results, _ = client.scroll(
            collection_name=COLLECTION_NAME,
            scroll_filter=Filter(must=tag_conditions),
            limit=limit,
            with_payload=True,
        )
