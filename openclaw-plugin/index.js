/**
 * OpenClaw Memory (Qdrant) Plugin V2 — 增强版
 *
 * 改进：importance 自动分级、加权搜索、关键词搜索、噪音过滤、memory_stats
 * 向后兼容 V1 数据（无 importance 映射的记录视为 medium）
 */
...
const NOISE_PATTERNS = [
  /^\{"image_key":/,                   // 纯 image_key JSON
  /^img_v\d+_/,                        // 飞书图片 key
  /^\[media attached:/,                // 媒体附件标记
  /^\/Users\/\S+\.openclaw\/media\//,  // 本地媒体路径
];
...
class MemoryDB {
  ...
  async stats() {
    if (this.useMemoryFallback) {
      ...
      return { total: this.memoryStore.length, categories, importances, v1Count: 0 };
    }
    ...
    while (true) {
      ...
      for (const p of result.points) {
        ...
        const lvl = p.payload.importance_level;
        if (lvl) {
          importances[lvl] = (importances[lvl] || 0) + 1;
        } else {
          v1Count++;
    ...
    return { total, categories, importances, v1Count };
  }
}
...
export default function register(api) {
  ...
  if (db.useMemoryFallback) {
    api.logger.info(`memory-qdrant-v2: using in-memory storage`);
  } else {
    api.logger.info(`memory-qdrant-v2: using Qdrant at ${cfg.qdrantUrl}`);
    db.healthCheck().then(health => {
      if (!health.healthy) {
        api.logger.warn(`memory-qdrant-v2: Qdrant health check failed: ${health.error}`);
      } else {
        api.logger.info('memory-qdrant-v2: Qdrant connection verified');
      }
    }).catch(err => {
      api.logger.error(`memory-qdrant-v2: Health check error: ${err.message}`);
    });
  }

  api.logger.info('memory-qdrant-v2: plugin registered (V2 enhanced)');
  ...
  function createMemoryStoreTool() {
    return {
      name: 'memory_store',
      description: '保存重要信息到长期记忆（V2增强版：自动importance分级）',
      parameters: {
        type: 'object',
        properties: {
  ...
  function createMemorySearchTool() {
    return {
      name: 'memory_search',
      description: '智能搜索长期记忆（V2增强版：加权排序，重要记忆优先）',
      parameters: {
        type: 'object',
        properties: {
  ...
  function createMemoryStatsTool() {
    return {
      ...
      execute: async function(_id, _params) {
        ...
        if (stats.v1Count > 0) {
          lines.push(`  未分级(v1): ${stats.v1Count}`);
        }
  ...
  // 搜索 claude-memory 集合（cross-collection，文本过滤）
  function createSearchClaudeMemoryTool() {
    return {
      name: 'memory_search_claude',
      description: '搜索 claude-memory 集合（Claude 的历史对话记忆），使用关键词文本过滤，适合跨系统查询',
      parameters: {
        type: 'object',
        properties: {
      ...
      execute: async function(_id, params) {
        ...
        const collectionName = 'claude-memory';
        ...
        try {
          ...
          if (points.length === 0) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: true, message: `未在 claude-memory 中找到包含 "${keyword}" 的记录`, count: 0 }) }] };
          }
          ...
          return { content: [{ type: 'text', text: JSON.stringify({
            success: true,
            message: `在 claude-memory 中找到 ${points.length} 条包含 "${keyword}" 的记录:\n\n${items}`,
            count: points.length,
  ...
  for (const tool of tools) {
    api.logger.info(`memory-qdrant-v2: registering ${tool.name}`);
  ...
  if (cfg.autoRecall) {
    api.on('before_agent_start', async (event) => {
      ...
      try {
        ...
        if (isTimeRecall) {
          api.logger.info('memory-qdrant-v2: 检测到时间性回忆请求');
          ...
          if (scored.length > 0) {
            api.logger.info('memory-qdrant-v2: 注入 ' + scored.length + ' 条记忆（时间+语义融合）');
            return {
        ...
        // 关键词搜索兜底：如果向量结果分数均低于 0.5，追加关键词搜索
        ...
        api.logger.debug(`memory-qdrant-v2: 注入 ${scored.length} 条记忆（加权排序，最高分 ${bestScore.toFixed(2)}）`);

        return {
          prependContext: formatRelevantMemoriesContext(scored)
        };
      } catch (err) {
        api.logger.warn(`memory-qdrant-v2: recall 失败: ${err.message}`);
      }
    });
  }

  if (cfg.autoCapture) {
    api.logger.info('memory-qdrant-v2: autoCapture enabled');
    api.on('agent_end', async (event) => {
      ...
      try {
        ...
        if (isNoiseContent(userText)) {
          api.logger.info(`memory-qdrant-v2: noise filtered: ${userText.slice(0, 60)}...`);
          return;
        }
        ...
        if (containsPII(combined) && !cfg.allowPIICapture) {
          api.logger.warn(`memory-qdrant-v2: Skipping text with PII`);
          return;
        }

        const vector = await embeddings.embed(combined);

        const existing = await db.search(vector, 1, SIMILARITY_THRESHOLDS.DUPLICATE);
        if (existing.length > 0) return;

        // V2: 自动检测分类和 importance
        ...
        const tag = { high: '★', medium: '☆', low: '·' }[importanceLevel];
        api.logger.info(`memory-qdrant-v2: captured [${tag}${category}] ${combined.slice(0, 80)}...`);
      } catch (err) {
        api.logger.warn(`memory-qdrant-v2: capture 失败: ${err.message}`);
      }
    });
  }
  ...
  api.registerCli(({ program }) => {
    const memory = program.command('memory-qdrant-v2').description('Qdrant 记忆插件 V2 命令');
    ...
    memory.command('stats').description('显示统计').action(async () => {
      ...
      if (stats.v1Count > 0) {
        console.log(`  未分级(v1): ${stats.v1Count}`);
      }
    });

    memory.command('search <query>').description('搜索记忆（加权排序）').action(async (query) => {
      const vector = await embeddings.embed(query);
      const results = await db.search(vector, 5, SIMILARITY_THRESHOLDS.LOW);
      const scored = results.map(r => {
        const level = r.entry.importance_level || 'medium';
